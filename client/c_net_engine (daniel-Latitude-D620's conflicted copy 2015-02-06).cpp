...
static void net_engine_function(std::string function){
	if(function == "reset_vector"){
		while(array_vector.size() != 0){
			std::string type = array_vector[0]->data_type;
			if(type == "coord_t"){
				delete (coord_t*)array_vector[0]->pointer;
			}else if(type == "model_t"){
				delete (model_t*)array_vector[0]->pointer;
			}else if(type == "input_buffer_t"){
				delete (input_buffer_t*)array_vector[0]->pointer;
			}else if(type == "render_buffer_t"){
				delete (render_buffer_t*)array_vector[0]->pointer;
			}else if(type == "client_t"){
				delete (client_t*)array_vector[0]->pointer;
			}else{
				printf("type is '%s'. I can't work with this\n", type.c_str());
			}
		}
	}
}
...
static void net_engine_parse(std::string a){
	if(a.find_first_of(ARRAY_ITEM_SEPERATOR_START) != std::string::npos){
		...
		net_engine_function(a.substr(a.find_first_of(ARRAY_FUNCTION_START)+1, a.find_first_of(ARRAY_FUNCTION_END)-a.find_first_of(ARRAY_FUNCTION_START)-2));
	}else{
