...
void input_t::blank(){
	is_used = false;
	for(unsigned int i = 0;i < INPUT_BUFFER_SIZE;i++) input_buffer[i] = NULL;
}

int input_t::init(int argc,char** argv){
...
int input_t::loop(){
	...
	for(unsigned int i = 0;i < INPUT_BUFFER_SIZE;i++){
		if(input_buffer[i] != NULL && input_buffer[i]->int_data != NULL){
	...
	while(SDL_PollEvent(&event)){ // the only input that works without a screen would be SDL_QUIT (Ctrl-C).
		while(input_buffer[available_input] != NULL){
			available_input++;
		...
		case SDL_MOUSEMOTION:
			...
			if(term_if_true(input_buffer[available_input] == NULL,(char*)"input_buffer allocation") == TERMINATE){
			...
			if(term_if_true(input_buffer[available_input]->int_data == NULL,(char*)"input_buffer->int_data") == TERMINATE){
				return_value = TERMINATE;
				break;
			}
		...
		case SDL_KEYUP:
			if(true){
				int key_entry = -1;
				if(event.key.keysym.sym < INPUT_BUFFER_SIZE && input_buffer[event.key.keysym.sym] != NULL){
			...
			if(true){
				...
				if(found_key == false){
					for(unsigned int i = 0;i < INPUT_BUFFER_SIZE;i++){
						if(input_buffer[i] != NULL){
							if(input_buffer[i]->type == INPUT_TYPE_KEYBOARD){
								assert(input_buffer[i]->int_data != NULL);
								if(input_buffer[i]->int_data[INPUT_TYPE_KEYBOARD_KEY] == event.key.keysym.sym){
									found_key = true;
									break;
								}
				...
				if(found_key == false){
					...
					if(term_if_true(input_buffer[available_input]->int_data == NULL,(char*)"input_buffer->int_data allocation") == TERMINATE){
						return_value = TERMINATE;
						break;
					}
...
void input_t::standard_input_check(){
	for(unsigned int i = 0;i < INPUT_BUFFER_SIZE;i++){
		if(input_buffer[i] != NULL){
			if(input_buffer[i]->type == INPUT_TYPE_MOUSE_MOTION){ // In order to have a mouse_motion, the screen has to be initialized and thus the renderer has to be initialized, thus nullifying the check
				...
				if(coord != NULL){
					coord->set_x_angle(true,y_angle_add);
					coord->set_y_angle(true,x_angle_add);
				}
				delete[] input_buffer[i]->int_data;
				input_buffer[i]->int_data = NULL;
				delete input_buffer[i];
...
bool input_t::query_key(input_buffer_t *buffer,int sdl_key,char key){ // This is used for debugging more than anything (all of the other code would be put into the main function here and not stranded in the classes in the code at random spots).
	...
	if(sdl_key == -1 && key == '\0' && buffer != NULL){
		for(unsigned int i = 0;i < INPUT_BUFFER_SIZE;i++){
			if(input_buffer[i] != NULL){
		...
		if(sdl_key < INPUT_BUFFER_SIZE && input_buffer[sdl_key] != NULL){
