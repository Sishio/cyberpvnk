...
static void net_send_data(){
	...
	std::string reset_vector = wrap(ARRAY_FUNCTION_START, "reset_vector", ARRAY_FUNCTION_END);
	for(unsigned long int i = 0;i < array_vector.size();i++){
		if(array_vector[i]->data_type == "client_t"){
		  net->write(reset_vector, ((client_t*)(array_vector[i]->pointer))->connection_info_id);
		}
	}
